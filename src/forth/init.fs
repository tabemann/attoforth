...
14 CELLS CONSTANT TERMINAL-INFO-SIZE
...
12 CELLS INPUT-ARG-OFFSET TERMINAL-HISTORY-LAST
13 CELLS INPUT-ARG-OFFSET TERMINAL-HISTORY-CURRENT
...
6 CELLS CONSTANT HISTORY-ENTRY-SIZE
0 CELLS OFFSET HISTORY-ENTRY-PREV
1 CELLS OFFSET HISTORY-ENTRY-NEXT
2 CELLS OFFSET HISTORY-ENTRY-DATA-SIZE
3 CELLS OFFSET HISTORY-ENTRY-BUFFER
4 CELLS OFFSET HISTORY-ENTRY-EDIT-DATA-SIZE
5 CELLS OFFSET HISTORY-ENTRY-EDIT-BUFFER
  ...
  DUP INPUT-COUNT @ 0> IF
    DUP TERMINAL-HISTORY-LAST @ 0<> IF
      ...
      OVER TERMINAL-HISTORY-CURRENT @ 0= OR IF
        ...
        OVER INPUT-COUNT @ = IF
          ...
          2 PICK INPUT-BUFFER @ 3 PICK INPUT-COUNT @ COMPARE 0<>
        ELSE
          TRUE
        THEN
      ELSE
    ...
    IF
      ...
      ELSE
        2DROP
      THEN
      0 OVER HISTORY-ENTRY-NEXT !
      2DUP SWAP TERMINAL-HISTORY-LAST !
...
: PRINT-FULL-LINE ( source -- )
  HIDE-CURSOR
  DUP DUP TERMINAL-INIT-ROW @ OVER TERMINAL-INIT-COLUMN @ GO-TO-COORD
...
: DROPS ( x*u u -- ) CELLS SP@ + SP! ;

: NIPS ( x*u u1 u -- u1 ) SWAP >R 1- DROPS R> ;
...
: COLON-END-ADDRESS ( xt -- addr )
  NAME>SECONDARY BEGIN DUP @ DUP 0<> WHILE
    DUP ['] (LITERAL) = OVER ['] BRANCH = OR SWAP ['] ?BRANCH = OR IF
      CELL+
    THEN
    CELL+
  REPEAT
...
: EXTRACT-BRANCH-DATA ( xt -- addr u )
  DUP ALLOCATE-BRANCH-DATA DUP 0 3 ROLL NAME>SECONDARY BEGIN DUP @ DUP 0<> WHILE
    DUP ['] (LITERAL) = IF
      DROP CELL+
    ELSE DUP ['] BRANCH = SWAP ['] ?BRANCH = OR IF
      CELL+ DUP @ 3 ROLL DUP ROT SWAP 4 ROLL ADD-TO-BRANCH-DATA ROT
    THEN THEN
    CELL+
  REPEAT
  2DROP NIP ;
...
: DECOMPILE ( xt -- )
  DUP EXTRACT-BRANCH-DATA ROT NAME>SECONDARY BEGIN DUP @ DUP 0<> WHILE
    ...
    DUP ['] (LITERAL) = IF
      ...
      THEN
    ELSE DUP ['] BRANCH = SWAP ['] ?BRANCH = OR IF
      CELL+ 2 PICK 2 PICK 2 PICK @ SEARCH-BRANCH-DATA IF
        ." <" (.) ." > "
      ELSE
        DROP DUP @ .
      THEN
...
:NONAME
  ...
  FORTH-WORDLIST DUP 1 SET-ORDER SET-CURRENT SP0 SP! QUIT ; EXECUTE