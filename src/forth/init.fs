  ...
  0 ?DO
    ...
    DUP 2 PICK INPUT-ARG @ TERMINAL-WIDTH @ >= IF
      ...
      DUP 3 PICK INPUT-ARG @ TERMINAL-HEIGHT @ >= IF
        ...
        THEN
      THEN
      0 GO-TO-COORD 2 PICK INPUT-ARG @ TERMINAL-ROW !
    ELSE
      CSI [CHAR] C EMIT
    THEN
    OVER INPUT-ARG @ TERMINAL-COLUMN !
...
: PRINT-DELETE-FORWARD ( source -- )
  ...
  DUP INPUT-COUNT @ OVER INPUT-ARG @ TERMINAL-INSERT-OFFSET @ - GO-BACKWARD ;
...
: HANDLE-KILL ( source -- )
  DUP INPUT-COUNT @ OVER INPUT-ARG @ TERMINAL-INSERT-OFFSET @ > IF
    DUP PRINT-KILL
    DUP INPUT-ARG @ TERMINAL-INSERT-OFFSET @ SWAP INPUT-COUNT !
  ELSE DROP THEN ;

: HANDLE-REFRESH ( source -- ) PRINT-REFRESH ;
...
:NONAME ( source c -- continue )
  ...
  THEN THEN THEN THEN THEN THEN ; ' HANDLE-CHAR DEFER!
