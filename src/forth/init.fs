...
2 CELLS OFFSET KILL-ENTRY-SIZE
...
: LAST-KILL-ENTRY-SIZE ( source -- addr )
  TERMINAL-KILL-RING-LAST @ DUP ABORT" Empty kill ring!" KILL-ENTRY-SIZE ;

: LAST-KILL-ENTRY-BUFFER ( source -- c-addr )
  TERMINAL-KILL-RING-LAST @ DUP ABORT" Empty kill ring!" KILL-ENTRY-BUFFER ;
...
: REGISTER-FORWARD ( source u -- )
  0 ?DO
    DUP TERMINAL-COLUMN@ 1+
    DUP 2 PICK TERMINAL-WIDTH @ >= IF
      DROP 0
      OVER TERMINAL-ROW @ 1+
      2 PICK TERMINAL-HEIGHT @ >= IF
        OVER TERMINAL-HEIGHT @ 1-
...
: GO-FORWARD ( source u -- )
  0 ?DO
    ...
    DUP 2 PICK TERMINAL-WIDTH @ >= IF
      DROP 0
      OVER TERMINAL-ROW @ 1+
      DUP 3 PICK TERMINAL-HEIGHT @ >= IF
        DROP OVER TERMINAL-HEIGHT @ 1-
        ...
        THEN
      THEN
      0 GO-TO-COORD 2 PICK TERMINAL-ROW !
    ELSE
      CSI [CHAR] C EMIT
    THEN
    OVER INPUT-ARG @ TERMINAL-COLUMN !
...
: GO-BACKWARD ( source u -- )
  0 ?DO
    ...
    ELSE
      ...
      THEN
      DUP TERMINAL-WIDTH @ 1- OVER INPUT-ARG @ TERMINAL-COLUMN !
    THEN
  LOOP
  DROP ;
...
: PRINT-YANK ( source -- )
  ...
  OVER LAST-KILL-ENTRY-SIZE @ - GO-BACKWARD ;

: PRINT-REFRESH ( source -- )
  DUP INPUT-ARG @ TERMINAL-INIT-ROW @ OVER INPUT-ARG @ TERMINAL-INIT-COLUMN @
...
: HANDLE-KILL-TO-PREV-SPACE ( source -- )
  ...
  DUP 0> IF
    ...
    4 PICK INPUT-COUNT @ 5 PICK TERMINAL-INPUT-OFFSET @ - MOVE
    2 PICK INPUT-COUNT @ OVER - 3 PICK INPUT-COUNT !
    2 PICK TERMINAL-INPUT-OFFSET @ OVER - 3 PICK TERMINAL-INPUT-OFFSET !
...
: HANDLE-YANK ( source -- )
  ...
  DUP TERMINAL-KILL-RING-LAST 0<> IF
    DUP LAST-KILL-ENTRY-SIZE @ TRY-EXPAND-TERMINAL-BUFFER
    DUP INPUT-BUFFER @ OVER TERMINAL-INSERT-OFFSET @ +
    OVER INPUT-BUFFER @ 2 PICK TERMINAL-INSERT-OFFSET @ +
    2 PICK LAST-KILL-ENTRY-SIZE @ + 2 PICK LAST-KILL-ENTRY-SIZE @ MOVE
    DUP LAST-KILL-ENTRY-BUFFER @
    OVER INPUT-BUFFER @ 2 PICK TERMINAL-INSERT-OFFSET @ +
    2 PICK LAST-KILL-ENTRY-SIZE @ MOVE
    DUP INPUT-COUNT @ OVER LAST-KILL-ENTRY-SIZE @ + OVER INPUT-COUNT !
    DUP PRINT-YANK
    DUP TERMINAL-INSERT-OFFSET @ OVER LAST-KILL-ENTRY-SIZE @ +
...
: HANDLE-YANK-PREV ( source -- )
  DUP TERMINAL-PREV-COMMAND-TYPE @ YANK-COMMAND = IF
    DUP LAST-KILL-ENTRY-SIZE @
    DUP MOVE-LAST-KILL-ENTRY-TO-FIRST
    DUP LAST-KILL-ENTRY-SIZE @ OVER - DUP 0> IF
    ...
    2 PICK INPUT-BUFFER @ 3 PICK TERMINAL-INSERT-OFFSET @ +
    3 PICK LAST-KILL-ENTRY-SIZE @ - 3 PICK LAST-KILL-ENTRY-SIZE @ MOVE
    OVER INPUT-COUNT @ OVER + 2 PICK INPUT-COUNT !
    2DUP PRINT-YANK-PREV
    OVER TERMINAL-INSERT-OFFSET @ + SWAP TERMINAL-INSERT-OFFSET !
  ELSE
    DROP
  THEN ;
